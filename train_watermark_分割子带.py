...
class WatermarkINN(nn.Module):
    def __init__(self, in_channels=3, steps=4, wm_len=64, alpha=0.1, target_band="HL"):
        ...
        self.target_band = target_band  # 指定目标子带：LL/HL/LH/HH

        # 子带通道索引映射（3通道原图，DWT后总通道12=3*4）
        self.band_indices = {
            "LL": slice(0, 3),  # 0-2通道
            "HL": slice(3, 6),  # 3-5通道
            "LH": slice(6, 9),  # 6-8通道
    ...
    def embed(self, image, watermark):
        ...
        bands = self.split_dwt_bands(coeffs)

        # 2. 仅对目标子带做INN处理
        target_band = bands[self.target_band]
        z = self.inn_forward(target_band)
        # 注入水印
        wm_feat = self.get_wm_feature(watermark, z.shape)
        z_watermarked = z + self.alpha * wm_feat
        # INN反向 -> 带水印的目标子带
        bands[self.target_band] = self.inn_inverse(z_watermarked)

        # 3. 重组所有子带 + IDWT
        coeffs_watermarked = self.concat_dwt_bands(bands)
...
def train_watermark(opt, dataloader, gaussians):
    ...
    # 2. 初始化 INN（指定目标子带，默认HL）
    inn_model = WatermarkINN(
        in_channels=3,  # 目标子带通道数（3通道）
        wm_len=opt.wm_len,
        alpha=0.1,
        target_band=opt.target_band  # 从命令行指定子带
    ).cuda()
    ...
    # 保存INN时记录目标子带（方便测试时匹配）
    torch.save({
        'state_dict': inn_model.state_dict(),
        'target_band': inn_model.target_band,
        'alpha': inn_model.alpha,
        'wm_len': inn_model.wm_len
    }, os.path.join(ckpt_dir, "watermark_inn.pth"))
...
if __name__ == '__main__':
    ...
    # 新增：指定目标子带
    parser.add_argument('--target_band', type=str, default='LL', choices=['LL', 'HL', 'LH', 'HH'],
                        help="Watermark embed target band (LL/HL/LH/HH)")
