...
def _preprocess_imgs(basedir, dir_name='images', factor=None, width=None, height=None, check_fn=lambda x: True):
    ...
    img0_list = [os.path.join(target_dir, f) for f in sorted(os.listdir(target_dir)) if check_fn(f, 0)]
    if len(img0_list) == 0:
        return None, None
...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True, fg_mask=False, use_depth=False,
    ...
    # Check for lesion masks (png preferred)
    check_lesionimg_fn = lambda f, i: f.endswith('png') or f.endswith('jpg') or f.endswith('JPG')
...
class EndoDataset:
    def __init__(self, opt, device, type='train'):
        ...
        self.lesion_masks = lesion_masks[self.start_index:self.end_index]
        ...
        self.lesion_masks = torch.from_numpy(self.lesion_masks)
